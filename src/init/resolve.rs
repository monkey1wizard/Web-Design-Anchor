...
/// Fallback reason: resolution succeeded, but installing one of the pinned direct
/// dependencies afterwards failed. Distinct from the seven resolution-time branches
/// above so a G11 report can tell "Spectrum 2 never resolved" apart from "Spectrum 2
/// resolved, then its pinned dependency would not install".
pub(crate) const FALLBACK_REASON_DEPENDENCY_INSTALL_FAILED: &str =
    "Spectrum 2 resolved, but installing a pinned direct dependency failed";
...
pub(crate) fn builtin_fallback(reason: &str) -> ResolvedDesignSystem {
    ResolvedDesignSystem {
