    ...
    Tz::Offset: std::fmt::Display,
{
    ...
    let resolved_ds = resolved_ds.unwrap_or_else(resolve::resolve_design_system);
    ...
    // rolls back the entire write plan too, with `package.json`/`deno.lock` in the
    // cleanup set alongside it (idempotent via `real_remover`'s NotFound-is-Ok
    // handling even when `deps::add` never got far enough to create them), so init
    // leaves either a fully initialized project or an empty directory, never a
    // partial one.
    if resolved_ds.name == resolve::SPECTRUM_TWO_DESIGN_SYSTEM_NAME {
        ...
        for spec in &specs {
            if let Err(dep_err) = crate::deps::add(root, spec) {
                created.push(PathBuf::from("package.json"));
                created.push(PathBuf::from("deno.lock"));
                if let Err(cleanup_err) = rollback(root, &created, real_remover) {
                    return Err(cleanup_err);
                }
                return Err(ToolFault::new(
                    format!("Failed to add pinned Spectrum 2 dependency {spec}: {dep_err}"),
                    Some(root.to_path_buf()),
                ));
            }
        }
...
mod tests {
    ...
    fn test_spectrum_two_dependency_resolution_failure_rolls_back_to_minimal() {
        ...
        // The high-visibility report carries the dependency reason, whether the
        // seam surfaces it as a completed report or as a returned fault.
        let carries_dependency_reason = match &result {
            Ok((report, _)) => report
                .diagnostics
                .iter()
                .any(|d| d.message.to_lowercase().contains("dependency")),
            Err(fault) => format!("{fault}").to_lowercase().contains("dependency"),
        };
        assert!(
            carries_dependency_reason,
            "dependency-resolution failure must be traceable to a dependency reason, got: {:?}",
            result
        );
